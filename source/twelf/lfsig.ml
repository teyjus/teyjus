...
type signature = Signature of (string list ref * Lfabsyn.typefam Symboltable.table * (Symb.symbol * int) Symboltable.table)


(* print the signature by type family. for each type family print out 
   the corresponding objects in order *)
let string_of_sig (Signature(files, types, objmap)) =
...
let get_filenames (Signature(files, types, objmap)) = (!files)
let get_typetable (Signature(files, types, objmap)) = types
